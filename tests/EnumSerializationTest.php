...
it('can have a unique identifier string', function ($enum, $result) {
    expect($enum->serialize())->toBe($result);
})->with([
...
it('can get an instance from correct identifier', function ($enumClass, $uniqueId, $result) {
    expect($enumClass::unserialize($uniqueId))->toBe($result);
})->with([
...
it('throw an exception with wrong uniqueId format', function ($enumClass, $uniqueId) {
    expect(fn () => $enumClass::unserialize($uniqueId))->toThrow(InvalidSerializedValue::class, "'$uniqueId' has an invalid format");
})->with([
...
it('throw an exception with wrong namespace or class name', function ($enumClass, $uniqueId) {
    expect(fn () => $enumClass::unserialize($uniqueId))->toThrow(InvalidSerializedValue::class, "'$uniqueId' has a wrong Namespace or Class name");
})->with([
...
it('throw an exception with wrong case name', function ($enumClass, $uniqueId) {
    expect(fn () => $enumClass::unserialize($uniqueId))->toThrow(InvalidSerializedValue::class, "There is not 'MISSING' case on the enum");
})->with([
